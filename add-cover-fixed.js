...
async function addCoverToExistingBooks() {
  try {
    ...
    // Connect to MongoDB using the correct URL from .env
    ...
    // Show the books that will be processed
    console.log('\n📋 Books to be processed:');
    pdfBooks.forEach((book, index) => {
      console.log(`${index + 1}. "${book.title}" by ${book.author} (${book.bookFile.filename})`);
    });
    ...
    for (let i = 0; i < pdfBooks.length; i++) {
      ...
      try {
        ...
        if (!fs.existsSync(originalPath)) {
          ...
          // List files in uploads/books directory
          const uploadsDir = path.join(__dirname, 'uploads/books');
          if (fs.existsSync(uploadsDir)) {
            const files = fs.readdirSync(uploadsDir);
            console.log(`📂 Files in uploads/books: ${files.length} files`);
            
            // Try to find the file with a similar name
            const similarFile = files.find(file => 
              file.includes(book.bookFile.filename.split('-')[0]) || 
              book.bookFile.filename.includes(file.split('-')[0])
            );
            
            if (similarFile) {
              console.log(`🔍 Found similar file: ${similarFile}`);
              // Update the path to use the found file
              const newPath = path.join(uploadsDir, similarFile);
              if (fs.existsSync(newPath)) {
                console.log(`✅ Using file: ${similarFile}`);
        ...
        console.log(`📁 Original file found: ${path.basename(originalPath)}`);
        ...
        if (combineResult.success) {
          ...
          console.log(`📄 New file: ${combineResult.filename}`);
          console.log(`📏 New size: ${(combineResult.size / 1024).toFixed(2)} KB`);
          successCount++;
        } else {
    ...
    if (successCount > 0) {
      console.log(`\n📚 ${successCount} books now have beautiful cover pages as the first page!`);
      console.log(`🔄 Refresh your frontend to see the changes.`);
      console.log(`📖 When you download these books, the cover page will be the first page.`);
    }
